...
class Solution {
 public:
  ...
  int my_quicksort(int l, int r, int k) {
    if (l == r) return a[l];
    ...
    int len = j - l + 1;
    if (k <= len) return my_quicksort(l, j, k);
    return my_quicksort(j + 1, r, k - len);
  }

  int findKthLargest(vector<int> &nums, int k) {
    a = nums;
    int n = int(a.size());
    return my_quicksort(0, n - 1, n + 1 - k);
  }
};
...
int main(void) {
  ...
  cin >>= arr;
  cin >> k;
  auto res = a.findKthLargest(arr, k);
  cout << res << '\n';
