 ...
 public:
  ...
  long long maxTaxiEarnings(int n, vector<vector<int>> &rides) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      if (l >= 0 && l < len && b[l].first <= left) {
        pair<int, long long> tmp{right, b[l].second + val};

        if (!b.empty() && tmp.second <= b.back().second)
          ;
        else
          b.push_back(tmp);
      }
...
int main(void) {
  ...
  while (cin >> n) {
    ...
    VVI arr(row, VI(col));
    For(i, 0, row) cin >>= arr[i];

    ll res = a.maxTaxiEarnings(n, arr);
    dbg(res);
  }
