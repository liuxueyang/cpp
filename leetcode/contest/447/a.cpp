...
class Solution {
 public:
  int countCoveredBuildings(int n, vector<vector<int>> &a) {
    ...
    for (auto &v : a) {
      int x = v[0], y = v[1];
      auto &s1 = mpx[x], &s2 = mpy[y];
      auto it1 = s1.lower_bound(y), it2 = s1.upper_bound(y);
      bool flag{true};
      if (it1 == s1.begin() || it2 == s1.end()) flag = false;

      if (flag) {
        it1 = s2.lower_bound(x), it2 = s2.upper_bound(x);
        if (it1 == s2.begin() || it2 == s2.end()) flag = false;
      }
