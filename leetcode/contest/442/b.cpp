...
class Solution {
 public:
  int numberOfComponents(vector<vector<int>> &a, int k) {
    auto check = [&](VI &ve1, VI &ve2) {
      VB vis1(110), vis2(110);
      for (auto x : ve1) vis1[x] = true;
      for (auto x : ve2) vis2[x] = true;
      int ans{};
      For1(i, 1, 100) {
        if (vis1[i] && vis2[i]) ans++;
      }
      return ans >= k;
    };
...
int main(void) {
  ...
  while (cin >> n >> m) {
    VVI ve(n, VI(m));
    For(i, 0, n) { For(j, 0, m) cin >> ve[i][j]; }
    int k;
    cin >> k;

    int res = a.numberOfComponents(ve, k);
    dbg(res);
  }
