  ...
  long long minCost(int n, int m, vector<vector<int>> &a) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < m; j++) {
        ...
        ll cost = ll(i + 1) * (j + 1);
        if (i) {
          tmp1 = min(tmp1, d0[i - 1][j] + cost);
        }
        if (j) {
          tmp1 = min(tmp1, d0[i][j - 1] + cost);
        }
        d1[i][j] = min(d1[i][j], tmp1);

        ll tmp0{INFL};
        tmp0 = min(tmp0, d1[i][j] + a[i][j]);
        d0[i][j] = min(d0[i][j], tmp0);
      }
    }
