...
class Solution {
 public:
  vector<int> countMentions(int numberOfUsers, vector<vector<string>> &events) {
    VI ans(numberOfUsers), online(numberOfUsers);
    int n = numberOfUsers;
    ...
    for (auto &vs : events) {
      string typ = vs[0], tss = vs[1];
      int ts = stoi(tss);

      if (typ == "MESSAGE") {
        string mentions = vs[2];

        if (mentions == "ALL") {
          for (auto &x : ans) x++;
        } else if (mentions == "HERE") {
          For(i, 0, n) {
            if (online[i] <= ts) ans[i]++;
          }
        } else {
        ...
        string mentions = vs[2], user = mentions;
        int id = stoi(user);
        online[id] = ts + 60;
      }
    }
...
int main(void) {
  ...
  int k = 2;
  ve = {{"MESSAGE", "10", "id1 id0"},
        {"OFFLINE", "11", "0"},
        {"MESSAGE", "71", "HERE"}};
  ans = a.countMentions(k, ve);
  dbg(ans);  // 2, 2
