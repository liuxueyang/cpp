...
class Solution {
 public:
  void dfs(int u, int mx_wei, VB &vis, vector<vector<PII>> &g, VI &ins) {
    vis[u] = true;
    for (auto [v, w] : g[u]) {
      if (!vis[v] && w <= mx_wei) {
        ins[v]++;
        dfs(v, mx_wei, vis, g, ins);
      }
    }
  }
  ...
  int minMaxWeight(int n, vector<vector<int>> &edges, int threshold) {
    ...
    auto check = [&](int mx_wei) {
      VB vis(n + 10);
      VI ins(n + 10);
      dfs(0, mx_wei, vis, g, ins);
      For(i, 0, n) {
        if (!vis[i]) return false;
        if (ins[i] > threshold) return false;
      }
      return true;
    };
