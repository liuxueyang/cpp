...
class Solution {
 public:
  ...
  ll xorr(int l, int r) { return p[r] ^ p[l - 1]; }
  ...
  int minXor(vector<int> &a_, int k_) {
    n = SZ(a_);
    k = k_;
    a = VL(n + 10);
    For(i, 0, n) a[i + 1] = a_[i];
    d = VVL(n + 10, VL(k + 10, INFL));
    vis = VVB(n + 10, VB(k + 10));
    p = VL(n + 10);
