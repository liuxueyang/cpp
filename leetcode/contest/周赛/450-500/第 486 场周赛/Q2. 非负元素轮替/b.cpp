...
class Solution {
 public:
  vector<int> rotateElements(vector<int>& a, int k) {
    ...
    for (int i = 0, j = k - 1; i < j; i++, j--) swap(poses[i], poses[j]);
    for (int i = k, j = m - 1; i < j; i++, j--) swap(poses[i], poses[j]);
    for (int i = 0, j = m - 1; i < j; i++, j--) swap(poses[i], poses[j]);
...
int main(void) {
  ...
  cin >> k;
  auto res = a.rotateElements(arr, k);
  dbg(res);
