...
class Solution {
 public:
  ...
  ll dfs(int pos, bool bound, bool number) {
    if (pos == len) return number;

    if (!bound && number && dp[pos] != -1) return dp[pos];
    ...
    For1(i, low, up) { ans += dfs(pos + 1, bound && (i == up), true); }
    if (!bound && number) dp[pos] = ans;
