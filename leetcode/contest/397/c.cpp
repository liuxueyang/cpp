  ...
  int maxScore_bad(vector<vector<int>> &g) {
    ...
    Rof(i, 0, n) {
      Rof(j, 0, m) {
        ...
        For(k, j + 1, m) {
          if (d[i][k] != -INF)
            ckmax(cur, max(d[i][k], 0) + g[i][k] - g[i][j]);
        }

        For(k, i + 1, n) {
          if (d[k][j] != -INF)
            ckmax(cur, max(d[k][j], 0) + g[k][j] - g[i][j]);
        }
...
int main(void) {
  ...
  Solution a;
  vector<VI> p{{9, 5, 7, 3}, {8, 9, 6, 1}, {6, 7, 14, 3}, {2, 5, 3, 1}};
  auto res = a.maxScore(p);
  dbg(res);

  p = vector<VI>{{4, 3, 2}, {3, 2, 1}};
  res = a.maxScore(p);
  dbg(res);
