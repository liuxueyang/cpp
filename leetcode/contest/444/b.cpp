...
struct node {
  ...
  bool operator<(const node &rh) const {
    if (ts != rh.ts) return ts < rh.ts;
    if (src != rh.src) return src < rh.src;
    return dst < rh.dst;
  }
};
...
class Router {
 public:
  ...
  bool addPacket(int source, int destination, int timestamp) {
    node x(timestamp, source, destination);
  ...
  vector<int> forwardPacket() {
    ...
    if (pkg.empty()) return res;

    auto rem = pkg.front();
    pkg.pop();

    res = {rem.src, rem.dst, rem.ts};
    dst_pkg[rem.dst].pop_front();
    vis.erase(rem);
