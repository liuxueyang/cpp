...
class Solution1 {
 public:
  long long minCost(vector<int> &arr, vector<int> &brr, long long k) {
    ll ans{};
    int n = int(arr.size());
    vector<pair<int, int>> arr1, brr1;

    for (int i = 0; i < n; i++) {
      arr1.push_back({arr[i], i});
      brr1.push_back({brr[i], i});
    }
    ...
    bool ok{};
    for (int i = 0; i < n; i++) {
      ans += abs(arr1[i].first - brr1[i].first);
      if (arr1[i].second != brr1[i].second && !ok) {
        ok = true;
        ans += k;
      }
    }
...
class Solution {
 public:
  long long minCost(vector<int> &arr, vector<int> &brr, long long k) {
    ...
    ll tmp = k;
    for (int i = 0; i < n; i++) tmp += abs(arr[i] - brr[i]);
