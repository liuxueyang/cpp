...
class Solution {
 public:
  int findMaxVal(int n, vector<vector<int>>& restrictions, vector<int>& diff) {
    VI a(n, INF);
    a[0] = 0;
    for (auto& rec : restrictions) {
      a[rec[0]] = rec[1];
    }

    For(i, 0, n - 1) { ckmin(a[i + 1], a[i] + diff[i]); }
    Rof(i, 0, n - 1) { ckmin(a[i], a[i + 1] + diff[i]); }
...
int main(void) {
  ...
  cin >> m1 >> m2;
  VVI arr(m1, VI(m2));
  For(i, 0, m1) For(j, 0, m2) cin >> arr[i][j];
  int len;
  cin >> len;
  VI brr(len);
  for (auto& x : brr) cin >> x;
  auto res = a.findMaxVal(n, arr, brr);
  cout << res << '\n';
