...
class Solution {
 public:
  vector<int> countServers(int n, vector<vector<int>> &logs, int x,
    ...
    for (auto &vec : logs) {
      a.push_back({vec[1], vec[0]});
    }

    vector<int> qs(queries);
    ...
    for (int i = 0, j = 0; j < len; j++) {
      auto [ts, serv_id] = a[j];
      if (serv_id != INF) vis[serv_id]++;

      while (i < j && ts - a[i].first > x) {
        auto [ts1, serv_id1] = a[i];
        if (serv_id1 != INF) {
          if (--vis[serv_id1] == 0) vis.erase(serv_id1);
        }

        i++;
      }
      if (serv_id == INF) {
        mp[ts] = n - int(vis.size());
      }
    }
...
int main(void) {
  ...
  while (cin >> n) {
    ...
    VI queries(n2);
    cin >>= queries;
    auto res = a.countServers(n, logs, x, queries);
    dbg(res);
  }
