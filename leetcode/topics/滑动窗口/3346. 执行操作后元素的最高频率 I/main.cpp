...
class Solution {
 public:
  int maxFrequency(vector<int> &a, int k, int m) {
    ...
    for (auto x : s) {
      int cnt1 = lower_bound(a.begin(), a.end(), x) -
                 lower_bound(a.begin(), a.end(), x - k);
      int cnt2 = upper_bound(a.begin(), a.end(), x + k) -
                 upper_bound(a.begin(), a.end(), x);
      int cnt = min(m, cnt1 + cnt2);
      ans = max(ans, cnt + mp[x]);
    }
...
int main(void) {
  ...
  while (cin >> n) {
    ...
    cin >>= arr;
    cin >> k >> m;
    auto res = a.maxFrequency(arr, k, m);
    dbg(res);
  }
