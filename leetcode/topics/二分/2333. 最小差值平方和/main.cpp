...
class Solution {
 public:
  long long minSumSquareDiff(vector<int> &nums1, vector<int> &nums2, int k1,
    ...
    for (auto [x, y] : mp) q.push({x, y});
    q.push({0, 0});

    while (tot > 0 && q.size() > 1) {
      auto [x1, cnt1] = q.top();
      q.pop();
      
      auto [x2, cnt2] = q.top();

      if (tot >= 1LL * cnt1 * (x1 - x2)) {
        tot -= 1LL * cnt1 * (x1 - x2);
        q.pop();
        q.push({x2, cnt1 + cnt2});
      }
      else {
        int m = tot / cnt1, rem = tot % cnt1;
        q.push({x1 - m, cnt1 - rem});
        q.push({x1 - m - 1, rem});
...
int main(void) {
  ...
  while (cin >> n) {
    ...
    ll ans = a.minSumSquareDiff(arr, brr, k1, k2);
    dbg(ans);
  }
