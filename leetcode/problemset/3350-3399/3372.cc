...
class Solution {
 public:
  ...
  int dfs(int u, int fa, int k, VVI &g) {
    if (k < 0) return 0;
    int res{1};
  ...
  vector<int> maxTargetNodes(vector<vector<int>> &edges1,
    ...
    int P{0};
    For(i, 0, m) {
      int tmp = dfs(i, -1, k - 1, g2);
      ckmax(P, tmp);
    }

    VI ans(n);
    For(i, 0, n) { ans[i] = dfs(i, -1, k, g1) + P; }
...
int main(void) {
  ...
  Solution a;
  VVI g1{{0, 1}, {0, 2}, {2, 3}, {2, 4}},
      g2{{0, 1}, {0, 2}, {0, 3}, {2, 7}, {1, 4}, {4, 5}, {4, 6}};
  VI res;

  res = a.maxTargetNodes(g1, g2, 2);
  dbg(res);
