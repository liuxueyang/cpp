...
class Solution {
 public:
  int maximumWhiteTiles(vector<vector<int>> &tiles, int carpetLen) {
    sort(all(tiles), [&](const VI &x, const VI &y) { return x[0] < y[0]; });
    int ans{}, n{SZ(tiles)}, l{}, r{}, cur{};

    for (int i = 0, j = 0; j < n; ++j) {
      r = tiles[j][1];
      cur += tiles[j][1] - tiles[j][0] + 1;

      while (i < j && r - carpetLen + 1 > tiles[i][1]) {
...
int main(void) {
  ...
  ve = {{1, 5}, {10, 11}, {12, 18}, {20, 25}, {30, 32}};
  res = a.maximumWhiteTiles(ve, 10);
  dbg(res);
