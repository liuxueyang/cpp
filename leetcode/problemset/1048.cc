...
class Solution {
 public:
  int longestStrChain(vector<string> &a1) {
    int n{SZ(a1)}, ans{1};
    VS a(n + 10);
    VI d(n + 10, 1);

    sort(all(a1), [](const string &s, const string &t) {
...
int main(void) {
  ...
  Solution a;
  VS ve{"a", "b", "ba", "bca", "bda", "bdca"};
  auto res = a.longestStrChain(ve);
  dbg(res);

  ve = {"xbc", "pcxbcf", "xb", "cxbc", "pcxbc"};
  res = a.longestStrChain(ve);
  dbg(res);
