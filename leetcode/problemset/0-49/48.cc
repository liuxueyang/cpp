...
class Solution {
 public:
  void rotate(vector<vector<int>> &a) {
    ...
    auto cycle = [&](int x, int y) {
      double x1 = x - m, y1 = y - m;
      swap(x1, y1);
      y1 = -y1;
      x1 += m, y1 += m;
...
int main(void) {
  ...
  Solution a;
  vector<VI> ve{{1, 2, 3}, {4, 5, 6}, {7, 8, 9}};
  // a.rotate(ve);
  // for (auto &v : ve) {
  //   dbg(v);
  // }

  ve = {{5, 1, 9, 11}, {2, 4, 8, 10}, {13, 3, 6, 7}, {15, 14, 12, 16}};
  a.rotate(ve);
  for (auto &v : ve) {
    dbg(v);
  }
