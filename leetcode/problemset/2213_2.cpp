...
struct info {
  ...
  info() : pre(0), post(0), sum(0), left(0), right(0) {}
  info(int pos) : pre(1), post(1), sum(1), left(pos), right(pos) {}
};
...
struct SegmentTree {
  ...
  info merge_info(const info &l, const info &r) {
    info ans;
    ...
    if (a[l.right] == a[r.left]) {
      if (l.post == l.right - l.left + 1)
        ans.pre += r.pre;
      if (r.pre == r.right - r.left + 1)
        ans.post += l.post;
      ans.sum = max(ans.sum, l.post + r.pre);
    }
  ...
  void update(int id) {
    ...
    seg[id].val = merge_info(seg[left].val, seg[right].val);
  }
