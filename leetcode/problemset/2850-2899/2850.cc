  ...
  int minimumMoves(vector<vector<int>> &g) {
    ...
    For(S, 1, 1 << n) {
      ...
      for (int S0 = S; S0; S0 -= (S0 & -S0)) {
        int y = S0 & -S0, j = __builtin_ctz(y), S1 = S ^ y;
        int cot = abs(a[i].f1 - b[j].f1) + abs(a[i].f2 - b[j].f2);
        ckmin(d[S], d[S1] + cot);
      }
    }
    return d[(1 << n) - 1];
...
int main(void) {
  ...
  Solution a;
  VVI ve{{1, 1, 0}, {1, 1, 1}, {1, 2, 1}};
  auto res = a.minimumMoves(ve);
  dbg(res);
