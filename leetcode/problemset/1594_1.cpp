...
public:
  ...
  int maxProductPath(vector<vector<int>> &b) {
    int n{SZ(b)}, m{SZ(b[0])};
    vector<VL> d1(n + 10, VL(m + 10)), a(n + 10, VL(m + 10)),
        d2(n + 10, VL(m + 10));

    For(i, 0, n) {
      For(j, 0, m) { a[i + 1][j + 1] = b[i][j]; }
    }

    d1[1][1] = d2[1][1] = a[1][1];

    For1(j, 2, m) { d1[1][j] = d2[1][j] = a[1][j] * d1[1][j - 1]; }
    For1(i, 2, n) { d1[i][1] = d2[i][1] = a[i][1] * d1[i - 1][1]; }
...
int main(void) {
  ...
  Solution a;
  vector<VI> ve{{-1, -2, -3}, {-2, -3, -3}, {-3, -3, -2}};
