...
class Solution {
 public:
  ...
  void Insert(string s) {
    ...
    for (auto c : s) {
      int id = c - 'a';
      if (!p->child[id]) p->child[id] = new node();
      p = p->child[id];
    }
...
int main(void) {
  ...
  Solution a;
  vector<string> inp{"a", "banana", "app", "appl", "ap", "apply", "apple"};
  auto res = a.longestWord(inp);
  dbg(res);
