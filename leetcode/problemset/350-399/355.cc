...
class Twitter {
 public:
  ...
  map<int, set<node>> tws;

  Twitter() : cur_ts{}, follower{}, fans{}, tws{} {}

  void postTweet(int userId, int tweetId) {
    node tw{cur_ts, tweetId};
    tws[userId].insert(tw);
    while (int(tws[userId].size()) > 10) {
      auto it = prev(tws[userId].end());
      tws[userId].erase(it);
    }
    cur_ts++;
  }
