...
public:
  long long countFairPairs(vector<int>& a, int lower, int upper) {
    ...
    For(i, 0, n) {
      int l = i + 1, r = n - 1, mid, x = a[i];
      while (l < r) {
        mid = (l + r) / 2;
        if (a[mid] >= lower - x) r = mid;
        else l = mid + 1;
      }
      
      int left = -1, right = -1;
      if (r < n && a[r] >= lower - x) left = r;
      else continue;
      
      l = i + 1, r = n - 1;
      while (l < r) {
        mid = (l + r + 1) / 2;
        if (a[mid] <= upper - x) l = mid;
        else r = mid - 1;
      }
      
      if (l < n && a[l] <= upper - x) right = l;
      else continue;
...
int main(void) {
  ...
  ve = {0,1,7,4,4,5};
  res = a.countFairPairs(ve, 3, 6);
  dbg(res);
  
  ve = {1,7,9,2,5};
  res = a.countFairPairs(ve, 11, 11);
  dbg(res);
