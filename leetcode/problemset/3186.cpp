...
public:
  long long maximumTotalDamage(vector<int> &a) {
    ...
    for (auto x : a) {
      ...
      ll pre{}, cur{};
      if (nemp(d) && d[l].f1 <= x - 3)
        pre = d[l].f2;
      if (nemp(d))
        cur = d.back().f2;
      ckmax(cur, pre + 1LL * cnt[x] * x);
      ckmax(ans, cur);
...
int main(void) {
  ...
  Solution a;
  VI ve{1, 1, 3, 4};
  auto res = a.maximumTotalDamage(ve);
  dbg(res);

  ve = {7, 1, 6, 6};
  res = a.maximumTotalDamage(ve);
  dbg(res);
