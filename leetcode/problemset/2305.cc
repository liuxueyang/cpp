    ...
    For(S, 1, 1 << n) {
      ...
      For1(i, 1, k) {
        ...
        for (int S0 = S; S0; S0 = (S0 - 1) & S) {
          ...
          int cnt1 = __builtin_popcount(S1);
          if (i - 1 > cnt1) continue;

          int tmp = max(d[S1][i - 1], sum);
          ckmin(d[S][i], tmp);
        }
      }
    }
...
int main(void) {
  ...
  Solution a;
  VI ve{8, 15, 10, 20, 8};
  auto res = a.distributeCookies(ve, 2);
  dbg(res);

  ve = {6, 1, 3, 2, 2, 4, 1, 2};
  res = a.distributeCookies(ve, 3);
  dbg(res);
