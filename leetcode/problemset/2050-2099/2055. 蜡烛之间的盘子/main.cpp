...
class Solution {
 public:
  vector<int> platesBetweenCandles(string s, vector<vector<int>> &queries) {
    ...
    vector<int> a(n + 10), bl(n + 10), br(n + 10, n + 10);
    ...
    for (int i = 0; i < m; i++) {
      auto &vec = queries[i];
      int left = vec[0] + 1, right = vec[1] + 1;
      int L = br[left], R = bl[right];

      if (L >= right || R <= left) continue;
      ans[i] = (R - L + 1) - (a[R] - a[L - 1]);
    }
    return ans;
  }
};
...
int main(void) {
  ...
  while (cin >> s) {
    ...
    VVI arr(n, VI(m));
    For(i, 0, n) cin >>= arr[i];
    auto res = a.platesBetweenCandles(s, arr);
    dbg(res);
  }
