...
class Solution {
 public:
  ...
  int findPaths(int n1, int m1, int maxMove, int stx_, int sty_) {
    n = n1, m = m1;
    stx = ++stx_, sty = ++sty_;
    d = {};
...
int main(void) {
  ...
  Solution a;
  auto res = a.findPaths(2, 2, 2, 0, 0);
  dbg(res);

  res = a.findPaths(1, 3, 3, 0, 1);
  dbg(res);
