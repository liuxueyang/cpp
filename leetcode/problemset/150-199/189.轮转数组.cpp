...
class Solution {
 public:
  void rotate2(vector<int> &nums, int k) {
    ...
    k %= n;
    Rof(i, n, 2 * n) { a[i] = a[i - k]; }
    For(i, 0, n) { nums[i] = a[i + n]; }
  }
  ...
  void rotate1(vector<int> &nums, int k) {
    ...
    for (int i = 0, j = k - 1; i < j; ++i, --j) swap(nums[i], nums[j]);
    for (int i = k, j = n - 1; i < j; ++i, --j) swap(nums[i], nums[j]);
