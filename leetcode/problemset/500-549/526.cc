    ...
    For(i, 1, n) {
      For(j, 0, 1 << n) {
        ...
        for (int j1 = j; j1; j1 -= (j1 & -j1)) {
          int id = __builtin_ctz(j1) + 1;
          int j2 = j ^ (j1 & -j1);
          if (id % (i + 1) == 0 || (i + 1) % id == 0) {
            d[i][j] += d[i - 1][j2];
          }
        }
      }
...
int main(void) {
  ...
  Solution a;
  auto res = a.countArrangement(2);
  dbg(res);

  res = a.countArrangement(1);
  dbg(res);

  res = a.countArrangement(3);
  dbg(res);
