...
class Solution {
 public:
  int minimumXORSum(vector<int> &a, vector<int> &b) {
    int n{SZ(a)};
    VVI d(n + 10, VI(1 << n, INF));

    For(i, 0, n) d[0][1 << i] = a[0] ^ b[i];
    For(i, 1, n) {
      For(j, 0, 1 << n) {
        if (__builtin_popcount(j) != i + 1) continue;
        for (int j1 = j; j1; j1 -= (j1 & -j1)) {
          int id = __builtin_ctz(j1 & -j1);
          int j2 = j ^ (1 << id);
          ckmin(d[i][j], d[i - 1][j2] + (a[i] ^ b[id]));
        }
      }
    }
    return d[n - 1][(1 << n) - 1];
...
int main(void) {
  ...
  Solution a;
  VI ve1{1, 2}, ve2{2, 3};
  auto res = a.minimumXORSum(ve1, ve2);
  dbg(res);
