...
class Solution {
 public:
  ...
  void dfs(TreeNode *root) {
    ...
    if (l) {
      g[l->val].push_back(val);
      g[val].push_back(l->val);
      dfs(l);
    }
    if (r) {
      g[r->val].push_back(val);
      g[val].push_back(r->val);
      dfs(r);
    }
  }

  int amountOfTime(TreeNode *root, int start) {
    g = vector<vector<int>>(1e5 + 10);
    begin_val = start;
...
int main(void) {
  ...
  root = lc.deserialize("1,5,3,null,4,10,6,9,2");
  start = 3;
  ans = a.amountOfTime(root, start);
  dbg(ans);

  root = lc.deserialize("1");
  start = 1;
  ans = a.amountOfTime(root, start);
  dbg(ans);
