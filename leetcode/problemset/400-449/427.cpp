...
class Solution {
 public:
  ...
  Node *dfs(int x1, int x2, int y1, int y2) {
    ...
    return new Node(false, false, dfs(x1, midx, y1, midy),
                    dfs(x1, midx, midy + 1, y2), dfs(midx + 1, x2, y1, midy),
                    dfs(midx + 1, x2, midy + 1, y2));
  }
  ...
  Node *construct(vector<vector<int>> &grid) {
    ...
    For(i, 0, n) {
      For(j, 0, n) { a[i + 1][j + 1] = grid[i][j]; }
    }

    b = a;
    For1(i, 1, n) {
      For1(j, 1, n) { b[i][j] += b[i][j - 1]; }
    }

    For1(j, 1, n) {
      For1(i, 1, n) { b[i][j] += b[i - 1][j]; }
    }
