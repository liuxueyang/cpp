    ...
    while (nemp(q)) {
      ...
      For(i, 0, 4) {
        ...
        if (x1 >= 0 && x1 < n && y1 >= 0 && y1 < m) {
          int tmp = max(d + 1, a[x1][y1] + 1);
          if (ckmin(dis[x1][y1], tmp)) {
            q.push(node(dis[x1][y1], x1, y1));
          }
        }
      }
    }
...
int main(void) {
  ...
  ve = {{94,79,62,27,69,84},{6,32,11,82,42,30}};
  res = a.minTimeToReach(ve);
  dbg(res);
