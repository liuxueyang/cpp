...
class Solution1 {
 public:
  ...
  TreeNode *lowestCommonAncestor(TreeNode *root, TreeNode *p, TreeNode *q) {
    vector<TreeNode *> path1{}, path2{}, cur_path{};
    ...
    int len1{SZ(path1)}, len2{SZ(path2)};
...
class Solution {
 public:
  TreeNode *lowestCommonAncestor(TreeNode *root, TreeNode *p, TreeNode *q) {
    ...
    if (val == pval || val == qval) return root;

    auto resl = lowestCommonAncestor(l, p, q),
         resr = lowestCommonAncestor(r, p, q);
    if (!resl) return resr;
    if (!resr) return resl;
