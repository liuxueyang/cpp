...
class Solution {
 public:
  vector<string> getWordsInLongestSubsequence(vector<string> &a1,
                                              vector<int> &b1) {
    int n{SZ(a1)}, ans{};
    VI d(n + 10), b(n + 10), path(n + 10);
...
int main(void) {
  ...
  Solution a;
  vector<string> ve1{"bab", "dab", "cab"};
  VI ve2{1, 2, 2};
  auto res = a.getWordsInLongestSubsequence(ve1, ve2);
  dbg(res);
