...
class Solution1 {
 public:
  int maximumBeauty(vector<int> &a, int k) {
    int mx = *max_element(a.begin(), a.end()) + 2 * k + 1;
    vector<int> b(mx + 10), c(mx + 10);
    ...
    c[0] = b[0];
    for (int i = 1; i <= mx; i++) c[i] = c[i - 1] + b[i];

    int ans = *max_element(c.begin(), c.begin() + mx + 1);
    return ans;
  }
};
