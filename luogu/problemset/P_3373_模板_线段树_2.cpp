...
struct tag {
  ll a, b;
  tag() : a(1), b(0) {}
  tag(ll a_, ll b_) : a(a_ % m), b(b_ % m) {}

  tag operator+(const tag &rh) const { return tag(a * rh.a, b * rh.a + rh.b); }
};
...
void solve() {
  ...
  while (q--) {
    ...
    if (op == 1) {
      cin >> x >> y >> k;
      tree.modify(1, 1, n, x, y, tag(k, 0));
    } else if (op == 2) {
      cin >> x >> y >> k;
      tree.modify(1, 1, n, x, y, tag(1, k));
    } else {
      cin >> x >> y;
