...
void solve() {
  ...
  while (q--) {
    ...
    if (op == "C") {
      ...
      For(i, 0, T) {
        if (i + 1 == C) {
          trees[i].modify(l, r, {0, 1});
        } else {
          trees[i].modify(l, r, {0, 0});
        }
      }
    } else {
      int ans{};
      For(i, 0, T) { ans += (trees[i].query(l, r).val > 0); }
      cout << ans << '\n';
    }
  }
}
