...
const int N = 200100;
int n, a[N], b[N], lo[N], d[30][N];

void Init() {
  For(i, 1, N) { lo[i] = log2(i); }
}
...
void solve() {
  ...
  cin >> n >> q;
  For1(i, 1, n) { cin >> a[i]; }
  For(i, 1, n) { b[i] = abs(a[i + 1] - a[i]); }

  For(j, 1, n) { d[0][j] = b[j]; }
  For1(i, 1, lo[n]) {
    for (int j = 1; j + (1 << i) - 1 < n; j++) {
      d[i][j] = gcd(d[i - 1][j + (1 << (i - 1))], d[i - 1][j]);
    }
  }
  auto query = [&](int l, int r) {
    int len = r - l + 1, i = lo[len];
    return gcd(d[i][l], d[i][r - (1 << i) + 1]);
  };
