...
void solve() {
  ...
  for (int i1 = 1, i2 = 1, j = 1; j <= n; j++) {
    ...
    if (SZ(mp1) == k) {
      int left = max(j - R + 1, i1), right = min(j - L + 2, i2);
      if (right >= left) {
        ans += right - left;
      }
    }
...
void solve2() {
  ...
  for (int i = L; i <= n; i++) {
    ...
    int left = max(b1[i], i - R + 1), right = min(b2[i] - 1, i - L + 1);
    if (right >= left) {
      ans += right - left + 1;
    }
  }
...
void solve1() {
  ...
  for (int i = 1; i <= n; i++) {
    if (b1[i]) {
      int left = max(i + L - 1, b1[i]), right = min(i + R, b2[i]);
      if (right > left)
        ans += right - left;
    }
  }
