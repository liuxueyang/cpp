...
void solve() {
  ...
  auto check = [&](int v) {
    VI b(n + 1), p(n + 1), pmie(n + 1, INF), pmie_idx(n + 1);
    pmie[0] = 0;
  ...
  int l = 0, r = 3e5, mid, left{}, right{};

  while (l < r) {
    mid = (l + r + 1) / 2;
    auto [posl, posr] = check(mid);

    if (posl != -1) {
      left = posl, right = posr;
      l = mid;
    } else
      r = mid - 1;
  }
