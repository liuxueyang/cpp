...
inline void INPUT_ELEMENTS(T &coll, const int n, const int start = 0) {
    ...
    copy_n(istream_iterator<typename T::value_type>(cin), n,
           back_inserter(coll));
}
...
void solve() {
    ...
    sort(c.begin(), c.end(), [&](int i, int j) { return b[i] < b[j]; });

    ll ans = b[c[0]] + b[c[1]];
    map<int, int> mp;

    for (int i = 0; i < n; i++) {
        for (auto x : factorials[a[i]]) {
            if (has(mp, x) && mp[x] > 0) {
                cout << "0\n";
                return;
            }
