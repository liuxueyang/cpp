...
void solve() {
  ...
  vector<PII> pre(n + 2, PII(INF, -INF)), post(n + 2, PII(INF, -INF));
  For1(i, 1, n) {
    pre[i].first = min(pre[i - 1].first, a[0][i]);
    pre[i].second = max(pre[i - 1].second, a[0][i]);
  }
  Rof1(i, 1, n) {
    post[i].first = min(post[i + 1].first, a[1][i]);
    post[i].second = max(post[i + 1].second, a[1][i]);
  }
  ...
  For1(i, 1, n) {
    int tmpl = min(pre[i].first, post[i].first),
        tmpr = max(pre[i].second, post[i].second);
    ckmax(mxL[tmpr], tmpl);
  }
