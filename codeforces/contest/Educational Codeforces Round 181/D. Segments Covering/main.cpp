...
struct node {
  int l, r, p, q;
  bool operator<(const node& rh) const { return PII{r, l} < PII{rh.r, rh.l}; }
};

static ll qmod(ll a, ll b, ll m = MOD1) {
  if (b == 0) return 1 % m;
  ll res{1};
  while (b) {
    if (b & 1) res = res * a % m;
...
static ll rmod(ll x, ll m = MOD1) { return qmod(x, m - 2, m); }
static ll rational_mod(ll p, ll q, ll m = MOD1) { return p * rmod(q) % m; }

void solve() {
  int n, m;

  while (cin >> n >> m) {
    vector<vector<node>> brr(m + 10);
    vector<node> arr;
    For(i, 0, n) {
      int l, r, p, q;
      cin >> l >> r >> p >> q;
    ...
    For1(i, 1, m) {
      ...
      for (auto& [l, r, p, q] : brr[i]) {
        psum[r] = psum[r] * rational_mod(q - p, q) % MOD1;
      }
    }
    ...
    for (auto &[l, r, p, q] : arr) {
      ll tmp = psum[r] * rmod(psum[l - 1]) % MOD1;
      tmp = tmp * rational_mod(p, q - p) % MOD1;
      d[r] = (d[r] + d[l - 1] * tmp % MOD1) % MOD1;
    }
