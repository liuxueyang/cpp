...
int main() {
  ...
  while (cin >> n >> s) {
    ...
    for (x = 1; x <= n; ++x) a[x] = a[x - 1] + (s[x] == '1' ? -3 : 1);
    k = *min_element(a, a + n + 1);
    for (x = 0; x <= n; ++x) a[x] -= k - 1;
    m = *max_element(a, a + n + 1);
    for (i = 0; i <= n; ++i) {
      for (x = a[i] - 2; x > 0; x -= x & -x) ans += c[x];
      for (x = a[i]; x <= m; x += x & -x) ++c[x];
    }
    for (i = 0; i <= m; ++i) c[i] = 0;
    for (i = 0; i <= n; ++i) ans += c[a[i] + 1], ++c[a[i]];
    printf("%lld\n", ans);
  }
  return 0;
}