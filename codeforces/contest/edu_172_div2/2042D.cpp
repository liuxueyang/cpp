...
void solve() {
  ...
  VI right(n, -1);
  sort(all(a),
       [&](const node &lh, const node &rh) -> bool { return lh.l < rh.l; });

  multiset<int> s_right;
  For(i, 0, n) {
    int L = i, R = i;
    s_right.insert(a[L].r);
    while (R + 1 < n && a[R + 1].l == a[L].l) {
      s_right.insert(a[++R].r);
    }

    For1(j, L, R) {
      auto it = s_right.lower_bound(a[j].r);
      ++it;
      if (it != s_right.end()) {
        right[a[j].id] = *it;
      }
  ...
  VI left(n, -1);
  sort(all(a),
       [&](const node &lh, const node &rh) -> bool { return lh.r < rh.r; });

  multiset<int> s_left;
  Rof(i, 0, n) {
    int R = i, L = i;
    s_left.insert(a[R].l);
    while (L - 1 >= 0 && a[L - 1].r == a[R].r) {
      s_left.insert(a[--L].l);
    }

    For1(j, L, R) {
      auto [it1, it2] = s_left.equal_range(a[j].l);
      if (next(it1) != it2)
        left[a[j].id] = *it1;
      else {
        if (it1 != s_left.begin()) left[a[j].id] = *prev(it1);
      }
    }
    i = L;
  }
  ...
  For(i, 0, n) {
    ...
    if (!(left[id] == -1 || right[id] == -1)) {
      ans[id] = a[i].l - left[id] + right[id] - a[i].r;
    }
  }
