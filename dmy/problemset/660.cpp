...
struct tag {
  ll a, b;
  tag() : a(1), b(0) {}
  tag(ll a_, ll b_) : a(a_ % MOD), b(b_ % MOD) {}
...
struct info {
  ll sum, len;
  info() : sum(0), len(0) {}
  info(ll x, int len_) : sum(x % MOD), len(len_) {}
};

tag operator+(const tag &l, const tag &r) {
  return tag(l.a * r.a % MOD, (l.b * r.a + r.b) % MOD);
}

info operator+(const info &l, const tag &t) {
  return info(l.sum * t.a % MOD + t.b * l.len, l.len);
}
...
void solve() {
  ...
  while (q--) {
    ...
    if (op == 1) {
      cin >> l >> r >> d;
      tr.modify(1, 1, n, l, r, tag(1, d));
    } else if (op == 2) {
      cin >> l >> r >> d;
      tr.modify(1, 1, n, l, r, tag(d, 0));
    } else if (op == 3) {
      cin >> l >> r >> d;
      tr.modify(1, 1, n, l, r, tag(0, d));
    } else {
      cin >> l >> r;
      auto ans = tr.query(1, 1, n, l, r);
      cout << ans.sum % MOD << '\n';
    }
  }
}
