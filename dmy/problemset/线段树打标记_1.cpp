...
struct tag {
  ...
  tag operator+(const tag &rh) const { return tag(d + rh.d); }
};
...
struct info {
  ...
  info operator+(const info &rh) const { return info(max(val, rh.val)); }
  info operator+(const tag &t) const { return info(val + t.d); }
};
