...
const int INF = 0x3f3f3f3f, MOD3 = 1e9 + 7, MOD1 = 998244353;
...
void solve() {
  ...
  VL fac(n + 10), rev_fac(n + 10);
  fac[0] = 1;
  For1(i, 1, n) { fac[i] = fac[i - 1] * i % M; }
  rev_fac[n] = qmod(fac[n], M - 2, M);
  Rof1(i, 0, n - 1) { rev_fac[i] = rev_fac[i + 1] * (i + 1) % M; }

  VL da(n + 10);
  da[2] = m * (m - 1) % M;

  For1(i, 3, n) {
    da[i] = qmod(m - 1, i - 1, M) * m % M;
    da[i] = (da[i] + M - da[i - 1]) % M;
  }
