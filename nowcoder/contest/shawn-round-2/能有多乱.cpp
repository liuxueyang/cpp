...
void Init() {
  fac[0] = 1;
  For1(i, 1, 1e5 + 5) { fac[i] = fac[i - 1] * i % MOD; }
}
...
void solve() {
  ...
  For1(i, 1, n) {
    cin >> a[i];
    p[i] = p[i - 1] + a[i];
    p1[i] = (p1[i - 1] + C(a[i])) % MOD;
  }

  auto get = [&](ll val) {
    return lower_bound(all1(p.begin(), n), val) - p.begin();
  };
  ...
  For(i, 0, T) {
    ...
    sum = (C(p[pl] - l + 1) + sum) % MOD;
    sum = (sum + C(r - p[pr - 1])) % MOD;
    sum = ((sum + p1[pr - 1]) % MOD - p1[pl] + MOD) % MOD;

    ll ans = (C(r - l + 1) - sum + MOD) % MOD;
