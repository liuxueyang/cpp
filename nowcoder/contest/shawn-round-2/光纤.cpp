...
void solve() {
  ll n, r, h, x0, y0, vx, vy;

  while (cin >> n >> r >> h >> x0 >> y0 >> vx >> vy) {
    auto square = [](ll x) { return x * x; };

    ll a = square(vx) + square(vy), b = 2 * x0 * vx + 2 * y0 * vy,
       c = square(x0) + square(y0) - square(r);

    lll delta = lll(b) * b - lll(4) * a * c;
    long double t1 = (sqrtl(delta) - b) / (2 * a), t2 = sqrtl(delta) / a;

    long double low = t1 + (n - 1) * t2, high = t1 + n * t2;
    long double vlow = h / high, vhigh = h / low;
