...
VL qinv(VL a, ll p) {
  ...
  For(i, 1, n) { s[i] = s[i - 1] * a[i] % p; }
  sv[n - 1] = qpow(s[n - 1], p - 2, p);
  Rof(i, 0, n - 1) sv[i] = sv[i + 1] * a[i + 1] % p;

  av[0] = sv[0];
  Rof(i, 1, n) { av[i] = sv[i] * s[i - 1] % p; }
  return av;
}
...
void solve1() {
  ...
  Rof1(i, 1, n) { av[i] = sv[i] * s[i - 1] % p; }

  For1(i, 1, n) { cout << av[i] << '\n'; }
}
