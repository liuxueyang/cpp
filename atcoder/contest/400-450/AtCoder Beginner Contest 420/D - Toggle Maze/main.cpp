...
void solve() {
  ...
  while (cin >> n >> m) {
    ...
    vector<VVI> dis(n, VVI(m, VI(2, INF)));
    vector<VVB> vis(n, VVB(m, VB(2)));
      ...
      For(i, 0, 4) {
        ...
        if (x1 >= 0 && x1 < n && y1 >= 0 && y1 < m && a[x1][y1] != '#') {
          if (a[x1][y1] == '?') {
            int z1 = z ^ 1;
            if (!vis[x1][y1][z1]) {
              ckmin(dis[x1][y1][z1], base + 1);
              vis[x1][y1][z1] = true;
              q.emplace(x1, y1, z1);
            }
          } else if ((a[x1][y1] == 'o' && (z % 2 == 0)) ||
                     (a[x1][y1] == 'x' && (z % 2 == 1)) || a[x1][y1] == '.' ||
                     a[x1][y1] == 'G' || a[x1][y1] == 'S') {
            if (!vis[x1][y1][z]) {
              ckmin(dis[x1][y1][z], base + 1);
              vis[x1][y1][z] = true;
              q.emplace(x1, y1, z);
            }
          }
    ...
    auto [x, y] = target;
    int ans = min(dis[x][y][0], dis[x][y][1]);
    cout << (ans == INF ? -1 : ans) << '\n';
  }
}
