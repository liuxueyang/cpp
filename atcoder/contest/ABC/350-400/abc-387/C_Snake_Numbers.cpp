...
ll dfs(int pos, bool bdl, bool bdr, bool is_num, int head) {
  if (pos == n) return is_num;

  if (!bdl && !bdr && is_num && dp[pos][head] != -1) return dp[pos][head];
  int low = 0, up = 9, l1 = s1[pos] - '0', r1 = s2[pos] - '0';
  ll ans{};

  if (!is_num && s1[pos] == '0') {
    ans = dfs(pos + 1, bdl, false, false, -1);
    low = 1;
  }

  if (bdl) ckmax(low, l1);
  if (bdr) ckmin(up, r1);
  if (is_num) ckmin(up, head - 1);

  For1(i, low, up) {
    ll tmp =
        dfs(pos + 1, bdl && i == l1, bdr && i == r1, true, (is_num ? head : i));
    ans += tmp;
  }

  if (!bdl && !bdr && is_num) dp[pos][head] = ans;
  return ans;
}
...
void solve() {
  ...
  n = SZ(s2);
  int len1{SZ(s1)};
  if (n > len1) s1 = string(n - len1, '0') + s1;
  memset(dp, -1, sizeof dp);

  ll ans = dfs(0, true, true, false, -1);
  cout << ans << '\n';
}
