...
struct node {
  ...
  bool operator<(const node &rh) const { return this->val() < rh.val(); }

  ll id() const { return 1LL * i + 1LL * n * j + 1LL * n * n * k; }
};
...
void solve() {
  ...
  For1(i, 1, n) cin >> b[i];
  For1(i, 1, n) cin >> c[i];
  sort(all1(a, n), greater<>());
  sort(all1(b, n), greater<>());
  sort(all1(c, n), greater<>());
