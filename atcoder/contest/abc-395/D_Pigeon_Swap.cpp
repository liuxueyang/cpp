...
void solve() {
  ...
  map<int, int> idx_nest, nest_idx, pig_idx;
  ...
  while (q--) {
    ...
    if (op == 1) {
      ...
      cin >> a >> b;
      int idxa = nest_idx[a], idxb = nest_idx[b];
      nest_idx[a] = idxb, nest_idx[b] = idxa;
      idx_nest[idxb] = a, idx_nest[idxa] = b;
    } else {
      cin >> a;
      int idx = pig_idx[a], nest = idx_nest[idx];
      cout << nest << '\n';
    }
  }
}
