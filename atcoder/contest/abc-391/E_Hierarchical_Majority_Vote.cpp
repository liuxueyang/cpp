...
void solve() {
  ...
  VI pw(n + 10, 1);
  For1(i, 1, 13) pw[i] = pw[i - 1] * 3;

  int m = pw[n];
  VVI a(2, VI(m + 10)), d(2, VI(m + 10, 1));
  int idx = 0;
  ...
  Rof1(i, 1, n) {
    int idx1 = idx ^ 1;
    For1(j, 1, pw[i - 1]) {
      int j1 = j * 3 - 2, j2 = j * 3 - 1, j3 = j * 3;

      if (a[idx][j1] == a[idx][j2]) {
        a[idx1][j] = a[idx][j1];
        if (a[idx][j1] == a[idx][j3]) {
          d[idx1][j] = d[idx][j1] + d[idx][j2] + d[idx][j3] -
                       max({d[idx][j1], d[idx][j2], d[idx][j3]});
        } else {
          d[idx1][j] = min({d[idx][j1], d[idx][j2]});
        }
      } else {
        a[idx1][j] = a[idx][j3];

        if (a[idx][j3] == a[idx][j2]) {
          d[idx1][j] = min({d[idx][j3], d[idx][j2]});
        } else {
          d[idx1][j] = min({d[idx][j3], d[idx][j1]});
        }
      }
    }
    idx = idx1;
