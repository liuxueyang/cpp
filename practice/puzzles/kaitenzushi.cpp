...
void Init() { __MultipleTestCase = 0; }

int getMaximumEatenDishCount(int N, vector<int> D, int K) {
  set<pair<int, int>> s;
  int ans{};
  map<int, int> mp;                                  // i -> D[i]
  priority_queue<int, vector<int>, greater<int>> q;  // min pq

  for (int i = 0; i < N; i++) {
    pair<int, int> cur{D[i], -1};

    auto it = s.lower_bound(cur);
    if (it != s.end() && it->first == D[i]) continue;
...
void solve() {
  ...
  int ans = getMaximumEatenDishCount(n, a, k);
  dbg(ans);
}
