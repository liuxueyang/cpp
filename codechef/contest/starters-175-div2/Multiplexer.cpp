...
void solve() {
  ...
  For1(i, 1, n) { g[a[i]].pb(i); }

  int ans{};
  unordered_map<int, int> cnt;

  auto merge = [&](const VI &va, const VI &vb) {
    VI res;
    int sum{SZ(vb)}, i = 0, j = 0, mx{};
    ...
    For(i, 1, szr) {
      ckmax(d[i], max(res[i], res[i] + d[i - 1]));
      ckmax(mx, d[i]);
    }
  ...
  for (auto &[val, vb] : g) {
    if (x > 1 && val % x == 0 && has(cnt, val / x)) {
      int tmp = merge(g[val / x], vb);
      ckmax(ans, tmp);
    }
  }
