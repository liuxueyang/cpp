...
struct Info {
  ...
  Info operator+(const Info& rh) const { return {msk | rh.msk}; }

  Info operator+(const Tag& t) const { return {t.val}; }
};
...
void solve() {
  ...
  while (m--) {
    ...
    if (op == "C") {
      cin >> color;
      tree.modify(1, 1, n, l, r, {(1 << color)});
    } else {
      auto res = tree.query(1, 1, n, l, r);
      cout << __builtin_popcount(res.msk) << '\n';
    }
  }
}
