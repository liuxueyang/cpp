...
// URL: https://vjudge.net/problem/%E6%B4%9B%E8%B0%B7-P2574
// Memory Limit: 125 MB
// Time Limit: 1000 ms
//
...
struct Tag {
  ...
  bool empty() const { return flip == 0; }
  void clear() { flip = 0; }
  Tag operator+(const Tag& rh) const { return {(flip + rh.flip) & 1}; }
};
struct Info {
  int cnt, len;

  Info operator+(const Info& rh) const { return {cnt + rh.cnt, len + rh.len}; }
...
struct SegmentTree {
  ...
  void modify(int id, int l, int r, int ql, int qr) {
    ...
    if (qr <= mid)
      ...
      modify(lc, l, mid, ql, mid);
      modify(rc, mid + 1, r, mid + 1, qr);
    }
    update(id);
  }
