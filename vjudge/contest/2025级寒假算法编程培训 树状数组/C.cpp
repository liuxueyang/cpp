// Problem: C - 线段树 1
// Contest: Virtual Judge - 2025级寒假算法编程培训 树状数组
// URL: https://vjudge.net/contest/784013#problem/C
// Memory Limit: 512 MB
// Time Limit: 1000 ms
//
...
PII child(int i) { return {i * 2, i * 2 + 1}; }

struct SegmentTree {
  Node tree[N * 4];

  SegmentTree() { For(i, 0, N * 4) tree[i] = {}; }

  void update(int id) {
    auto [lc, rc] = child(id);
    tree[id].info = tree[lc].info + tree[rc].info;
  }
...
void solve() {
  ...
  while (m--) {
    ...
    if (op == 1) {
      cin >> x >> y >> k;
      tree.modify(1, 1, n, x, y, k);
    } else {
      cin >> x >> y;
      auto res = tree.query(1, 1, n, x, y);
      cout << res.sum << '\n';
    }
  }
}
