...
int Hash(int l, int r) { return a[r] - a[l - 1] * p[r - l + 1]; }

int common_len(int idx1, int idx2) {
  int len1 = n - idx1 + 1, len2 = n - idx2 + 1;
  int len = min(len1, len2);
  int l = 0, r = len, mid;
  while (l < r) {
    mid = (l + r + 1) / 2;
    if (Hash(idx1, idx1 + mid - 1) == Hash(idx2, idx2 + mid - 1))
      l = mid;
    else
      r = mid - 1;
  }
...
int compare(int idx1, int idx2) {
  ...
  if (len == 0)
    ...
    return s[idx1 + len] > s[idx2 + len];
}

void my_qsort(int l, int r) {
  if (l >= r) return;

  int i = l - 1, j = r + 1, x = b[l];
  while (i < j) {
    while (compare(x, b[++i]));
    while (compare(b[--j], x));
    if (i < j) swap(b[i], b[j]);
...
void solve() {
  ...
  random_device rd{};
  mt19937 rg(rd());
  shuffle(b + 1, b + 1 + n, rg);

  my_qsort(1, n);
  For1(i, 1, n) { cout << b[i] - 1 << ' '; }
  cout << '\n';

  VI height(n + 1);
  For1(i, 2, n) { height[i] = common_len(b[i], b[i - 1]); }
  For1(i, 1, n) { cout << height[i] << ' '; }
  cout << '\n';
}
