...
const int N = 100100, P = 99991, L = 6;
int n, idx, h[N], e[N], ne[N], snow[N][L];
bool st[N];
...
bool Insert(int* a) {
  ...
  for (int j = h[pos]; j != -1; j = ne[j]) {
    int ver = e[j];
    if (check(snow[ver], a)) return true;
  }
  memcpy(snow[idx], a, sizeof(int) * L);
  Add(pos, idx);
