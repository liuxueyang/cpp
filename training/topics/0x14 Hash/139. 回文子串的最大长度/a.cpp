...
void solve() {
  ...
  while (cin >> s) {
    ...
    b[n + 1] = 0;
    Rof1(i, 1, n) { b[i] = b[i + 1] * Pr + (s[i] - 'a'); }

    auto H1 = [&](int l, int r) { return a[r] - a[l - 1] * p1[r - l + 1]; };
    auto H2 = [&](int l, int r) { return b[l] - b[r + 1] * p1[r - l + 1]; };
    auto check1 = [&](int idx, int len) {
      return H1(idx, idx + len) == H2(idx - len, idx);
    };
    auto check2 = [&](int idx, int len) {
      return H1(idx, idx + len - 1) == H2(idx - len, idx - 1);
    };
