...
struct Node {
  ...
  bool operator!=(Node rh) const {
    return is_op != rh.is_op || op != rh.op || num != rh.num;
  }
};
...
void solve() {
  ...
  For(i, 0, n) {
    if (isdigit(s[i])) {
      ...
      while (nemp(stk) && stk.top() != Node('(')) {
        ans.pb(Node{stk.top()});
        stk.pop();
      }
      if (nemp(stk)) stk.pop();
    } else {
      while (nemp(stk) && prio[stk.top().op] >= prio[s[i]]) {
        ans.pb(stk.top());
        stk.pop();
      }
      stk.push(Node(s[i]));
