...
void solve() {
  ...
  For(i, 0, cnt) {
    if (powers[i]) {
      For(j, 0, powers[i]) { multi(res, primes[i]); }
    }
  }

  int len{SZ(res)};
  cout << res[len - 1];
  Rof(i, 0, len - 1) { cout << setw(4) << setfill('0') << res[i]; }
  cout << '\n';
}
