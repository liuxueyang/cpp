...
struct Node {
  double x, y;
  bool typ;
  Node(double x_, double y_, bool typ_) : x(x_), y(y_), typ(typ_) {}
  Node() : x(0), y(0), typ(false) {}

  bool operator<(const Node& rh) const { return x < rh.x; }

  double dist(const Node& rh) const {
    if (typ == rh.typ) return INFL;
    double dx = x - rh.x, dy = y - rh.y;
    return sqrt(dx * dx + dy * dy);
  }

  bool compy(const Node& rh) const { return y < rh.y; }
};
...
double dfs(int l, int r) {
  ...
  int mid = (l + r) >> 1;
  double midx = a[mid].x;
  double resl = dfs(l, mid), resr = dfs(mid + 1, r);
  ckmin(ans, min(resl, resr));

  int i = l, j = mid + 1;
  For1(k, l, r) {
    if (j > r || (i <= mid && a[i].compy(a[j])))
      b[k] = a[i++];
    else
      b[k] = a[j++];
  }
  ...
  int id = 0;
  For1(k, l, r) {
    if (b[k].x >= midx - ans && b[k].x <= midx + ans) c[++id] = b[k];
  }

  For1(i, 1, id) {
    for (int j = i - 1; j >= 1 && ans - (c[i].y - c[j].y) > eps; j--) {
      ckmin(ans, c[i].dist(c[j]));
    }
  }
  return ans;
}
