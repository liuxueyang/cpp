...
ll solve2(int n, int p) {
  if (n == 0) return 1;

  if (n % 2 == 0) {
    ll res = solve2(n / 2 - 1, p) * (1 + qmod(p, n / 2)) % MOD1;
    res = (res + qmod(p, n)) % MOD1;
    return res;
  } else {
    ll res = solve2(n / 2, p) * (1 + qmod(p, (n + 1) / 2)) % MOD1;
    return res;
  }
}

ll solve3(int n, int p) {
  if (n == 0) return 1;

  if (n % 2 == 0) {
    // 和 solve2 相比，区别是偶数的分割方式不同
    ll res = solve2(n / 2 - 1, p) * (1 + qmod(p, n / 2 + 1)) % MOD1;
    res = (res + qmod(p, n / 2)) % MOD1;
    return res;
  } else {
    ll res = solve2(n / 2, p) * (1 + qmod(p, (n + 1) / 2)) % MOD1;
    return res;
