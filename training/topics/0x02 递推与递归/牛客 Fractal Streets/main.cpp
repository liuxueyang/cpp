...
pair<ll, ll> dfs(ll n, ll m) {
  if (n == 0) return make_pair(0, 0);
  ll d = (1LL << n), d1 = d >> 1;
  ll cnt = (1LL << (2 * n - 2));

  pair<ll, ll> res1 = dfs(n - 1, m % cnt);
  ll idx = m / cnt;
  ll x = res1.first, y = res1.second, x1, y1;
  if (idx == 0) {
    x1 = y, y1 = x;
  } else if (idx == 1) {
    x1 = x, y1 = y + d1;
...
void solve() {
  ...
  pair<ll, ll> p1 = dfs(n, x), p2 = dfs(n, y);
  ll dx = (p1.first - p2.first), dy = p1.second - p2.second;
  ll res = llround(sqrt(dx * dx + dy * dy) * 10);
  cout << res << '\n';
}
